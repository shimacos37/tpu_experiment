...
flags.DEFINE_enum(
    "model", None, MODELS.keys(), "Name of the model to be run", case_sensitive=False
)
flags.DEFINE_string("tpu", None, "Name of the TPU to use")
flags.DEFINE_integer("batch_size", 128, "Batch size to be used for model")
flags.DEFINE_integer("epochs", 10, "Number of training epochs")
flags.DEFINE_bool(
    "use_synthetic_data", False, "Use synthetic data instead of Cifar; used for testing"
)
  ...
  """
    ...
    """
        ...
        x_train = np.random.randn(
            self.num_train_images, image_size, image_size, 3
        ).astype(np.float32)
        y_train = np.random.randint(
            self.num_classes, size=self.num_train_images, dtype=np.int32
        )
        y_train = y_train.reshape((self.num_train_images, 1))

        x_test = np.random.randn(
            self.num_test_images, image_size, image_size, 3
        ).astype(np.float32)
        y_test = np.random.randint(
            self.num_classes, size=self.num_test_images, dtype=np.int32
        )
