...
flags.DEFINE_string(
    ...
    default=None,
    help=(
        "The directory where the resized (160x160) ImageNet input data is "
...
flags.DEFINE_bool(
    ...
    default=False,
    help=(
        "Enabling this uses a faster learning rate schedule along with "
...
def main(unused_argv):
    ...
    if FLAGS.mode == "train":
        ...
        if FLAGS.use_fast_lr:
            ...
            large_steps = int(
                min(50 * NUM_TRAIN_IMAGES / FLAGS.train_batch_size, FLAGS.train_steps)
            )
            ...
            resnet_classifier.train(
                input_fn=imagenet_train_large.input_fn, max_steps=large_steps
            )
        else:
            resnet_classifier.train(
        ...
        start_timestamp = tf.gfile.Stat(
            os.path.join(FLAGS.model_dir, "START")
        ).mtime_nsec
        results = []
        ...
        for step in ckpt_steps:
            ...
            tf.logging.info(
                "Eval results: %s. Elapsed seconds: %d" % (eval_results, eval_time)
            )
            results.append(
                [
                    current_epoch,
        ...
        with tf.gfile.GFile(
            os.path.join(FLAGS.model_dir, "results.tsv"), "wb"
        ) as tsv_file:  # pylint: disable=line-too-long
