...
flags.DEFINE_string(
    "model_dir", default="", help="The directory where the model will be stored."
)

flags.DEFINE_integer("train_batch_size", default=1024, help="Batch size for training.")

flags.DEFINE_integer("eval_batch_size", default=1024, help="Batch size for evaluation.")

flags.DEFINE_integer("num_shards", default=8, help="Number of shards (TPU cores).")

flags.DEFINE_integer(
    "iterations_per_loop",
    default=None,
    help=(
        "Number of interior TPU cycles to run before returning to the host. "
...
flags.DEFINE_integer(
    "prefetch_dataset_buffer_size",
    8 * 1024 * 1024,
    "Number of bytes prefetched in read buffer. 0 means no buffering.",
)

flags.DEFINE_integer(
    "num_files_infeed", 8, "Number of training files to read in parallel."
)

flags.DEFINE_integer(
    "shuffle_buffer_size", 1000, "Size of the shuffle buffer used to randomize ordering"
)
...
# For mode=eval
flags.DEFINE_integer(
    "min_eval_interval", default=180, help="Minimum seconds between evaluations."
)
...
flags.DEFINE_integer(
    "network_depth", default=121, help="Number of levels in the Densenet network"
)
...
def model_fn(features, labels, mode, params):
    ...
    current_epoch = tf.cast(global_step, tf.float32) / params["batches_per_epoch"]
...
def main(unused_argv):
    ...
    if FLAGS.mode == "train":
        ...
        while current_step < FLAGS.train_steps:
            next_checkpoint = min(
                current_step + steps_per_checkpoint, FLAGS.train_steps
            )
